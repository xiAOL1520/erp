...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public HSSFWorkbook exportExcel(){
        HSSFWorkbook warehouseshot=new HSSFWorkbook();
        String[] tableHeaders = {"库存id", "商品编号", "数量","进价","批次","出厂日期"};//表头
        // 创建工作表:Sheet1
        String name="库存快照 ";
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        Date now=new Date();
        String today= df.format(now);
        HSSFSheet sheet = warehouseshot.createSheet("库存快照 "+today);
        // 创建单元格样式
        HSSFCellStyle cellStyle = warehouseshot.createCellStyle();
        // 设置水平对齐方式:中间对齐
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        // 设置垂直对齐方式:中间对齐
        cellStyle.setVerticalAlignment(VerticalAlignment.CENTER);
        // 设置字体样式
        Font font = warehouseshot.createFont();
        font.setBold(true);
        ...
        beginCell.setCellValue("库存快照 "+today);
        ...
        for(int i=0;i< warehouseCountingVOS.size();i++){
            row = sheet.createRow(i + 2);
            WarehouseCountingVO temp=warehouseCountingVOS.get(i);
            row.createCell(0).setCellValue(temp.getId());
            row.createCell(1).setCellValue(temp.getProduct().getId());
            row.createCell(2).setCellValue(temp.getQuantity());
            row.createCell(3).setCellValue(temp.getPurchasePrice().doubleValue());
            row.createCell(4).setCellValue(temp.getBatchId());
            row.createCell(5).setCellValue(temp.getProductionDate());
        }
        return warehouseshot;
    }
}
